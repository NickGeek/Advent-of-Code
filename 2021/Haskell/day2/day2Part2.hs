...
data Pos = Pos { depth :: Int
               ...
               deriving (Show)
initialPos = Pos { depth = 0, hPos = 0, aim = 0 }
...
step :: Pos -> Task -> Pos
step pos (Forward x) = pos { hPos = (hPos pos) + x, depth = (depth pos) + ((aim pos) * x) }
step pos (Up x) = pos { aim = (aim pos) - x }
step pos (Down x) = pos { aim = (aim pos) + x }
